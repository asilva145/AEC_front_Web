...
function FormEditer(props){
  ...
  const [pokemonID , setclientID] = useState(props.location.search.substring(4,props.location.search.length));
  ...
  async function editPokemon(nom,photos,attaque1, attaque2) { 
    try{ 
      const response = await fetch('http://localhost:3001/client2/'+ pokemonID, { 
        method:'PUT', 
